...
it `observer-kubeconfig.yaml` (or adjust kaas service [configuration](./app/config.py) accordingly).
...
```bash
kind get kubeconfig --name observer > manifests/observer-kubeconfig.yaml
```
